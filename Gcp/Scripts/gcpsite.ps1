...
$sqlparam = "/QUIETSIMPLE /ACTION=install /INSTANCENAME=MSSQLSERVER /PID=$($sqlkey) /FEATURES=SQLEngine,Replication,IS,BC,Conn " + `
...
"/AGTSVCACCOUNT=domain\user /AGTSVCPASSWORD=$($d) /ISSVCACCOUNT=`"NT Authority\Network Service`" /NPENABLED=1"
