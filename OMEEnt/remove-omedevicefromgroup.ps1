...
function remove-omedevicefromgroup
{
    [CmdletBinding()]
    param
    (
    ...
    $adddvc = "https://$($server)/api/GroupService/Actions/GroupService.RemoveMemberDevices"
    ...
    {   
        ...
        if($mydevice -ne $Null)
        {
            ...
            write-host "Removed $ipaddress from $groupname" -f yellow
        }
        else
        {
    ...
    elseif($servicetag -ne $Null)
    {
        ...
        if($mydevice -ne $Null)
        {
            ...
            $payload = $payload.Replace('"[','[').replace(']"',']')
            invoke-restmethod -uri $adddvc -method Post -ContentType 'application/json' -body $payload -websession $mysess
            write-host "Removed $Servicetag from $groupname" -f yellow
        }
        else
        {
