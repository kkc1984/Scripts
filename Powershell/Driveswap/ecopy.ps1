...
if($drive)
{
    if(test-path ($path + 'E_xsfer.txt'))
    {
        remove-item ($path + 'E_xsfer.txt') -force
    }

    robocopy /E /Z /SEC /V /LOG+:c:\windows\temp\E_xsfer.txt "$($drive.driveletter):\" "$($drive.newletter):\"

    $completionchk = get-content ($path +  'E_xsfer.txt') -tail 20 | `
